...
\begin{itemize}
    \item O que é
    \item Competição
    \item Liberação do código fonte e a importância para a construção do
          framework
    \item Processo de escrita dos bots (C++)
    \item Simulação do input do jogador (ver paper)
    \item Informações recebidas sobre o ambiente
    \item Elementos para a simulação de um jogador humano
    \item Elementos de suporte ao desenvolvedor
    \begin{itemize}
        \item Ferramentas para ajudas nos testes
        \item Level editor
        \item Mapas de teste
        \item Bots de exemplo
    \end{itemize}
    \item Limitações
\end{itemize}
