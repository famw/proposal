...
jogos \textit{GameMaker} (Versão 8.0 Pro) - e lançado gratuitamente para a plataforma
...
A liberação do código fonte de Spelunky pode ser considerada um marco muito
importante, pois permitiu que fossem criadas modificações para o jogo. Estas
modificações, que podem ser encontradas no fórum oficial da \textit{Mossmouth}
-- empresa desenvolvedora de jogos criada por Derek Yu -- \cite{SPELUNKYFORUM},
são correções de \textit{bugs}, mapas customizados ou até mesmo modos de jogo
...
competição de inteligência artificial para o jogo. A aplicação se comunica com o
código-fonte original e permite a utilização das linguagens \textit{GML} (padrão
do GameMaker) e \textit{C++} para a descrição do comportamento de \textit{bots}.

A \textit{API} permite que o desenvolvedor resgate informações de objetos
...
tal, o \textit{framework} instaurou um sistema de \textit{fog of war}, limitando
o conhecimento do ambiente que pode ser obtido pela inteligência artificial.
Para objetos estáticos, uma vez que o jogador visualizou o objeto ele poderá
receber informações sobre ele permanentemente. Para objetos dinâmicos, o
\textit{bot} só poderá receber informações sobre eles se os mesmos estiverem
sendo visualizados pelo jogador.
...
O \textit{framework} também disponibiliza algumas ferramentas para suporte ao
desenvolvedor. Uma delas é a capacidade de receber informações sobre o estado do
\textit{bot}, sua trajetória ou o ambiente na tela do jogo enquanto o programa
está executando. Outra ferramenta muito valiosa é um editor de níveis. Isto faz
com que o desenvolvedor não precise esperar que um cenário específico ocorra no
jogo para testar uma capacidade específica do \textit{bot}. Além disso, o jogo
foi modificado para permitir que o jogador pegue atalhos e pule para qualquer
nível ou área do jogo, caso queira testar as capacidades de seu \textit{bot} em
níveis mais difíceis sem ter que jogar o jogo desde o início. Outra
funcionalidade muito útil é a habilidade de mudar a velocidade do jogo enquanto
joga. Isto facilitaria a implementação de uma abordagem focada em aprendizado de
máquina, tendo em vista que aceleraria o processo de aprendizado. Os criadores
também disponibilizam alguns \textit{bots} e níveis básicos para ajudar novos
desenvolvedores a entender sua \textit{API}.
